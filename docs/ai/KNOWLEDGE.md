...
### Đã gỡ bỏ hoàn toàn MoviePy (2026-08-21)
- **[Context]**: `moviepy` đã bị gỡ khỏi project. `core/engines/video_maker.py` không còn hàm `make_video()`/`_render_text_frame()` (engine render legacy PIL) — file này giờ CHỈ còn các hàm tiện ích dùng chung mà GSAP engine (`html_video_maker.py`) import (`_collect_visual_sources`, `_prepare_visual_background`, `_select_video_encoder`, `_mix_audio_with_ducking`/`_mix_audio_simple`, `_parse_srt`, `open_ffmpeg_with_log`, `check_ffmpeg_result`).
- **[Bài học quan trọng]**: Trước khi gỡ, tưởng GSAP engine độc lập với `moviepy` — thực tế nó phụ thuộc `moviepy` GIÁN TIẾP qua `video_maker.py` (đọc duration audio bằng `AudioFileClip`, chuẩn bị video nền bằng `VideoFileClip`/`ColorClip`/`concatenate_videoclips`) và cả TRỰC TIẾP trong chính `html_video_maker.py` (mix BGM fallback, `ColorClip`). Khi gỡ 1 dependency dùng chung giữa 2 engine, phải trace toàn bộ import graph trước, không chỉ xoá hàm "trông có vẻ" là của engine cũ.
- **[Fix Strategy]**: Thay `VideoFileClip` bằng `cv2.VideoCapture` (đọc frame thô, class `_Cv2VideoSource`/`_FormattedVideoSource`/`_ConcatVideoSource` trong `video_maker.py`), thay `AudioFileClip(...).duration` bằng `_get_audio_duration()` (ffprobe, đã có sẵn ở `tts.py`), thay mix BGM fallback (`CompositeAudioClip`+`audio_loop`) bằng `_mix_audio_simple()` (ffmpeg `amix` thường, không sidechain). `opencv-python-headless` thay `moviepy` trong `requirements.txt`.
