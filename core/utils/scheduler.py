...
from core.automation.uploader import upload_video, log_upload_result
from core.automation.nick_manager import _load_nicks, record_upload
...
def run_scheduler():
    """Run scheduler."""
    logger.info("🚀 Khởi động Scheduler Background...")
    logger.info("🕒 Chờ đến giờ vàng (11h-13h hoặc 18h-21h) để tự động đăng...")
        ...
        try:
            ...
            if is_golden_hour():
                ...
                for attempt in range(max_retries):
                    ...
                    if success:
                        break
                    logger.info(f"[SCHEDULER] ⚠️ Thử lại lần {attempt + 1} cho nick {nick}...")
                ...
                if queue:
                    ...
                    logger.info(f"[SCHEDULER] ⏳ Nghỉ {wait_time:.0f}s trước khi đăng tiếp...")
                    time.sleep(wait_time)
            else:
                time.sleep(60) # Kiểm tra mỗi phút nếu chưa tới giờ
        except Exception as e:
            logger.info(f"[SCHEDULER] ❌ Lỗi: {e}")
            time.sleep(60)
