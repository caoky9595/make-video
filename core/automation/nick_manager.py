...
def add_nick(nick_name: str, username: str = "", proxy: str = "") -> dict:
    ...
    if nick_name in nicks:
        logger.info(f"⚠️  Nick [{nick_name}] đã tồn tại.")
    ...
    logger.info(f"✅ Đã thêm nick [{nick_name}] | Profile: {profile_dir}")
    return nick_data
...
def remove_nick(nick_name: str):
    ...
    if nick_name in nicks:
        del nicks[nick_name]
        _save_nicks(nicks)
        logger.info(f"✅ Đã xóa nick [{nick_name}]")
    else:
        logger.info(f"⚠️  Nick [{nick_name}] không tồn tại")
...
def list_nicks(status_filter: Optional[str] = None) -> list:
    ...
    if not filtered:
        logger.info("📋 Chưa có nick nào.")
        return []

    logger.info(f"\n📋 Danh sách nick ({len(filtered)}):")
    logger.info("-" * 70)
    logger.info(f"{'Tên':12} {'Username':15} {'Trạng thái':10} {'Video':7} {'Hôm nay':8} {'Proxy':15}")
    logger.info("-" * 70)
...
def get_upload_plan() -> list:
    ...
    if not available:
        logger.info("📋 Không có nick nào có thể đăng hôm nay.")
        return []
    ...
    for nick in available:
        ...
        logger.info(f"  {nick['nick_name']:12} → {remaining} video (status: {nick['status']})")

    logger.info("-" * 40)
    logger.info(f"  Tổng: {total_slots} video cần chuẩn bị")
    return plan
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        logger.info("  python nick_manager.py list              # Xem tất cả nick")
        logger.info("  python nick_manager.py add <name> [user] # Thêm nick")
        logger.info("  python nick_manager.py remove <name>     # Xóa nick")
        logger.info("  python nick_manager.py plan              # Kế hoạch hôm nay")
        logger.info("  python nick_manager.py available          # Nick có thể đăng")
        sys.exit(0)
    ...
    if cmd == "list":
        ...
        for n in nicks:
            logger.info(f"  {n['nick_name']}: {n['_remaining']} slots left")
    else:
        logger.info("❌ Lệnh không hợp lệ")
