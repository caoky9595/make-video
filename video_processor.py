...
def download_video(url: str, output_name: Optional[str] = None) -> Optional[str]:
    ...
    print(f"  [Download] Đang tải: {url[:60]}...")

    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)

        if result.returncode != 0:
            print(f"  [Download] ❌ Lỗi yt-dlp: {result.stderr[:200]}")
            return None
        ...
        if result.returncode == 0:
            ...
            if downloaded:
                ...
                print(f"  [Download] ✅ Tải xong: {final_path}")
                return final_path

        print(f"  [Download] ❌ Lỗi: {result.stderr[:200]}")
        return None

    except subprocess.TimeoutExpired:
        print("  [Download] ❌ Timeout (quá 120 giây)")
        return None
    except FileNotFoundError:
        print("  [Download] ❌ yt-dlp chưa được cài. Chạy: pip install yt-dlp")
        return None
...
def process_video(
    ...
    if not os.path.exists(input_path):
        print(f"  [Process] ❌ Không tìm thấy: {input_path}")
        return None
    ...
    if not video_info:
        print("  [Process] ❌ Không đọc được thông tin video")
        return None
    ...
    print(f"  [Process] Video gốc: {width}x{height}, {duration:.1f}s")
    ...
    if crop_vertical and width > height:
        ...
        print(f"  [Process] Crop: {width}x{height} → {new_width}x{height}")
    ...
    print(f"  [Process] Saturation: {sat}, Brightness: {bri}")
    ...
    if hook_text:
        try:
            ...
            print(f"  [Process] 🗣️ Đã tạo TTS cho hook: {hook_text[:30]}...")
        except Exception as e:
            print(f"  [Process] ❌ Lỗi tạo TTS: {e}")
            tts_audio_path = None
    ...
    if not bg_music and shift_pitch:
        ...
        if semitones != 0:
            ...
            print(f"  [Process] Pitch shift: {semitones:+d} semitones")
    ...
    if bg_music and os.path.exists(bg_music):
        print(f"  [Process] 🎵 Thay thế bằng nhạc nền an toàn: {os.path.basename(bg_music)}")
    ...
    print(f"  [Process] Đang xử lý video...")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"  [Process] ✅ Xong: {output_path} ({output_size:.1f} MB)")
            return output_path
        else:
            print(f"  [Process] ❌ FFmpeg lỗi: {result.stderr[-300:]}")
            return None

    except subprocess.TimeoutExpired:
        print("  [Process] ❌ Timeout (quá 5 phút)")
        return None
    except FileNotFoundError:
        print("  [Process] ❌ FFmpeg chưa cài. Chạy: brew install ffmpeg")
        return None
