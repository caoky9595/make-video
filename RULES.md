...
- **Ngách duy nhất**: Sự Thật Thú Vị & Tâm Lý Cuộc Sống (hành vi, thói quen, cảm xúc, mối quan hệ). KHÔNG trộn ngách khác.
...
- **Video sự thật/tâm lý (không gắn giỏ)**: dùng clip tạo bằng Google Flow/Veo (trợ lý "Hoạt hình Veo thủ công" trong app) — app sinh sẵn prompt từng cảnh, người dùng tự dán vào Flow, tải video về rồi upload vào Media Nền theo đúng thứ tự cảnh. Không còn dùng stock Pexels/ảnh AI tự động (đã gỡ khỏi hệ thống).
- **Thứ tự cảnh**: đặt tên file clip bắt đầu bằng số thứ tự (`1_...`, `2_...`) khi upload để khớp đúng với timeline audio.
...
- **Hashtag chuẩn ngách**: #suthat #tamly #tamlyhoc #tiktokshop #xuhuong #fyp.
...
- **Pipeline video sự thật/tâm lý**: Script Gemini/Groq → TTS → Trợ lý sinh prompt cảnh → (thủ công: dán vào Google Flow, tải clip về, upload) → Render.
...
*Cập nhật pivot ngách: 2026-08-20 — chuyển từ Đồ Gia Dụng & Nhà Bếp Thông Minh sang Sự Thật Thú Vị & Tâm Lý Cuộc Sống (xem `channel_strategy.md` mục pivot ở đầu file). Các quy tắc chính sách/vận hành (nhãn AIGC, footage thật cho sản phẩm, không tự động upload, khung giờ đăng) không đổi.*
