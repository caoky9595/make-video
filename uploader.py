...
def login_tiktok(nick_name: str = "default"):
    ...
    try:
        ...
        print("❌ DrissionPage chưa được cài. Chạy: pip install DrissionPage")
        return False

    from nick_manager import _load_nicks
    nicks = _load_nicks()
    ...
    if proxy:
        ...
        co.set_proxy(proxy)
        print(f"   🌐 Đang dùng Proxy: {proxy}")

    print(f"\n🔐 Mở Chrome cho nick: [{nick_name}]")
    print(f"   Profile: {profile_path}")
    print("   Hãy đăng nhập TikTok trong cửa sổ vừa mở.")
    ...
    page.disconnect()
    print(f"✅ Cookie đã lưu trong profile: {profile_path} (Vui lòng tự đóng cửa sổ Chrome)")
    return True
...
def upload_video(
    ...
    if not any(profile_path.iterdir()):
        print(f"⚠️  Nick [{nick_name}] chưa đăng nhập. Đang mở đăng nhập...")
    ...
    try:
        ...
        if not upload_input:
            print("  ❌ Không tìm thấy ô upload file. TikTok có thể đã thay đổi UI.")
            return False

        upload_input.input(os.path.abspath(video_path))
        print(f"  ✅ Đã chọn file: {video_path}")
        _random_delay(DELAY_AFTER_UPLOAD)

        # === BƯỚC 2: Điền caption ===
        print("  [2/4] Điền caption...")
        try:
            ...
            if caption_box:
                ...
                print("  ⚠️  Không tìm thấy ô caption")

        except Exception as e:
            print(f"  ⚠️  Lỗi caption: {e}")

        _random_delay(DELAY_AFTER_CAPTION)

        # === BƯỚC 3: Gắn sản phẩm ===
        if product_id:
            print(f"  [3/4] Gắn sản phẩm ID: {product_id}...")
            try:
                    ...
                    if product_btn:
                        ...
                        if search_box:
                            ...
                            if add_btn:
                                ...
                                print(f"  ✅ Đã gắn sản phẩm: {product_id}")
                                _random_delay((1, 2))
                            else:
                                print(f"  ⚠️ Không tìm thấy sản phẩm {product_id}")
            except Exception as e:
                print(f"  ⚠️ Lỗi khi gắn sản phẩm: {e}")
        else:
            print("  [3/4] Bỏ qua gắn sản phẩm (không có product_id)")
        ...
        try:
            ...
            if post_btn:
                ...
                print("  ✅ Đã bấm nút Đăng! Đang chờ TikTok xử lý...")
                ...
                if confirm_btn:
                    ...
                    print("  ✅ Đã bấm nút Xác Nhận trên Popup!")
                    _random_delay((2, 4))
                ...
                if success_modal:
                    print("  ✅ Đã thấy thông báo đăng thành công từ TikTok!")
                    _random_delay((2, 4))
                else:
                    print("  ⚠️  Không thấy thông báo thành công, đang đợi thêm 15s để chắc chắn...")
                    time.sleep(15)
            else:
                print("  ⚠️  Không tìm thấy nút Đăng. Hãy đăng thủ công.")
                input("  Nhấn Enter sau khi đã đăng xong...")

        except Exception as e:
            print(f"  ⚠️  Lỗi khi bấm đăng: {e}")
...
def upload_queue(jobs: list) -> dict:
    ...
    for i, job in enumerate(jobs, 1):
        ...
        for attempt in range(max_retries):
            if attempt > 0:
                print(f"  ⚠️ Đang thử lại lần {attempt + 1}/{max_retries} cho nick [{nick}]...")
                _random_delay((10, 20))
        ...
        if i < len(jobs):
            ...
            print(f"  ⏳ Nghỉ {wait_time:.0f}s trước video tiếp theo...")
            time.sleep(wait_time)

    # Tổng kết
    print("\n" + "=" * 50)
    print(f"✅ Thành công: {len(results['success'])}/{len(jobs)}")
    print(f"❌ Thất bại:   {len(results['failed'])}/{len(jobs)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--login":
        ...
        print("Cách dùng:")
        print("  python uploader.py --login [nick_name]    # Đăng nhập cho 1 nick")
        print("  python uploader.py --test                 # Test upload 1 video")
