...
def _search_freetouse_tracks(query: str, limit: int = 8):
    ...
    if not base_url:
        print("  [Music AI] FREETOUSE_API_URL chưa cấu hình. Bỏ qua AI online.")
        return []
    ...
    try:
        ...
        print(f"  [Music AI] API search failed: {e}")
        return []
...
def _download_track(track: dict, output_dir: str = "audio_bg"):
    ...
    try:
        ...
        print(f"  [Music AI] Downloaded: {filename}")
        return out_path
    except Exception as e:
        print(f"  [Music AI] Download failed: {e}")
        return None


def pick_local_music_for_script(script_text: str, music_dir: str = "audio_bg"):
    """Pick the most suitable local music file based on keyword overlap."""
    ...
    print(f"  [Music AI] Local selected: {best_name}")
    return best_path
...
def resolve_music_for_script(
    ...
    """Resolve and download suitable music from online provider."""
    query = _extract_music_query(script_text, custom_query=music_query)
    print(f"  [Music AI] Query: {query}")

    if provider != "freetouse":
        print(f"  [Music AI] Provider '{provider}' chưa hỗ trợ.")
        return None
